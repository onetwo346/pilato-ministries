...
/* =========================================
MOBILE NAV DRAWER — set up first & defensively,
so a failure anywhere else in this file can never
prevent the menu / nav links from working.
========================================= */
...
try {
  ...
  console.error("Toast setup failed:", err);
}
...
try {
  ...
  console.error("Video tile setup failed:", err);
}
...
try {
  ...
  if ("IntersectionObserver" in window) {
    ...
    // (observer edge cases, etc.), don't leave it invisible forever.
    setTimeout(() => {
...
/* =========================================
POINTER-DRIVEN EFFECTS (desktop only — mouse required)
Cursor light, 3D orb tilt, card tilt, live-screen tilt, magnetic
buttons. These are pure visual enhancements; every part is wrapped
so a failure here can never affect navigation or other features.
========================================= */
...
try {
  ...
  console.error("Pointer effects setup failed:", err);
}